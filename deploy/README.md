...
前提：**PostgreSQL（或 MySQL）与 Redis 已在生产环境可用**，本编排只启动 **API** 与 **前端 Nginx**。
...
4. 首次启动时 API 容器会执行 **`prisma migrate deploy`**（PostgreSQL 且使用仓库内迁移时）。若使用 **MySQL**，请阅读仓库根目录 `README.md` 中 MySQL 说明，并可在 `.env` 中设置 `SKIP_PRISMA_MIGRATE=1` 后自行同步表结构。
...
构建 API 镜像时需与运行时一致，例如在 `deploy/.env` 中设置 `DATABASE_PROVIDER=mysql` 后重建；或单独构建：

```bash
docker build --build-arg DATABASE_PROVIDER=mysql -f backend/Dockerfile backend
```
...
目标：在已有 **Node.js** 的服务器上直接跑 **Nest API**，用 **Nginx（或同类）** 托管前端静态文件；**PostgreSQL / MySQL、Redis** 仍使用你已部署好的实例。
...
cd backend
cp .env.example .env
# 按生产填写 DATABASE_PROVIDER、DATABASE_URL 等（仅用于 prisma generate / migrate，可指向生产库或临时库）
npm ci
...
export DATABASE_PROVIDER=postgresql   # 或 mysql
...
- **首次或发版后**执行迁移（PostgreSQL 且使用仓库内迁移时）：
  ...
  MySQL 请先阅读仓库根 `README.md` 中 MySQL 说明；可能需要 `db push` 等，勿盲目对 MySQL 执行当前 PG 迁移目录。
