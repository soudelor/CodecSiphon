...
function extractApiErrorMessage(e: unknown): string | null {
  ...
  if (e.response?.status === 429) return t('errors.urlExtractRateLimited');
  const raw = e.response?.data as { message?: string | string[] };
  const m = raw?.message;
  if (typeof m === 'string' && m.trim()) return localizeMessage(m.trim());
  if (Array.isArray(m) && m.length)
    return m
      .filter((x) => typeof x === 'string' && x.trim())
      .map((x) => localizeMessage(x))
...
async function submit() {
  ...
  try {
    ...
    void loadCaptcha();
    errorMsg.value =
      extractApiErrorMessage(e) ?? t('errors.networkError');
  } finally {
    loading.value = false;
  }
}
  ...
  <div class="auth-page">
    ...
    <div class="card">
      <div class="hero">
        ...
        <p class="tag">{{ t('auth.forgotPasswordTagline') }}</p>
      </div>

      <template v-if="submitted">
        <p class="success">{{ t('auth.forgotPasswordDoneHint') }}</p>
        <div class="footer single">
          <RouterLink class="accent-link" to="/login">{{
            t('auth.backToLogin')
          }}</RouterLink>
        </div>
      </template>
